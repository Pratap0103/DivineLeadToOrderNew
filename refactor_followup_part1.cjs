...
function FollowUp() {
  ...
  // Helper function to check date filter condition
  const checkDateFilter = (followUp, filterArray) => {
    if (!filterArray || filterArray.length === 0) return true

    const checkSingleDateFilter = (followUp, filterType) => {
      if (activeTab === "pending") {
        ...
        switch (filterType) {
          case "today": return columnCLText.includes("today")
          case "overdue": return columnCLText.includes("overdue")
          case "upcoming": return columnCLText.includes("upcoming")
          default: return true
        }
      } else {
    ...
    return filterArray.some(type => checkSingleDateFilter(followUp, type))
  }
  ...
  const getHeaders = () => {
    if (activeTab === "pending") {
      ...
      if (isAdmin && isAdmin()) baseHeaders.push("Assigned To");
  ...
  const renderPendingRow = (followUp, index) => (
    <tr key={`${followUp.leadId}-${index}`} className="hover:bg-slate-50 transition-colors">
      ...
      {isAdmin && isAdmin() && <td className="px-3 sm:px-4 py-3 sm:py-4 text-sm text-gray-500 whitespace-nowrap">{followUp.assignedTo}</td>}
