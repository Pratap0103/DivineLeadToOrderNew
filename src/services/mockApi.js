...
export const mockApi = {
    ...
    fetchFollowUps: async (currentUser, isAdminFunc) => {
        ...
        const historyFollowUps = historySource.filter(row => {
            ...
            const historyObj = {
                timestamp: row.date,
                leadNo: row.leadNo || row.enquiryNo,
    ...
    fetchCallTrackers: async (currentUser, isAdminFunc) => {
        ...
        // Pending Enquiry Trackers (from FMS with hasPendingCallTracker = true)
    ...
    submitCallTracker: async (data) => {
        ...
        // When Enquiry is processed, we mark it done in FMS and move to Enquiry Tracker History
