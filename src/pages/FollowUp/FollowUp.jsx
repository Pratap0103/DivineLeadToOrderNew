...
function FollowUp() {
  ...
  const getHeaders = () => {
    if (activeTab === "pending") {
      const baseHeaders = [
        "Actions", "Call Date", "Lead No.", "Company Name", "Lead Source", 
        "Person Name", "Phone No.", "Location", "Customer Say", "Enquiry Status"
      ];
  ...
  const renderPendingRow = (followUp, index) => (
    <tr key={`${followUp.leadId}-${index}`} className="hover:bg-slate-50 transition-colors">
      ...
      <td className="px-3 sm:px-4 py-3 sm:py-4 text-sm text-gray-500"><div className="max-w-[120px] truncate" title={followUp.assignedTo}>{followUp.assignedTo}</div></td>
      ...
      {isAdmin && isAdmin() && <td className="px-3 sm:px-4 py-3 sm:py-4 text-sm text-gray-500 whitespace-nowrap">{followUp.personName}</td>}
