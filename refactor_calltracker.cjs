...
function CallTracker() {
  ...
  const activeData = activeTab === "pending" 
    ...
    : activeTab === "directEnquiry" 
      ? filteredDirectEnquiryPendingTrackers 
  ...
  const pendingHeaders = [
    "Actions", "Timestamp", "Lead No.", "Lead Receiver Name", "Lead Source",
    "Phone No.", "Salesperson Name", "Company Name", "Current Stage", "Calling Date"
  ];
  if (isAdmin()) pendingHeaders.push("Assigned To");
  pendingHeaders.push("Item/Qty");

  const directEnquiryHeaders = [
    "Actions", "Timestamp", "Lead No.", "Lead Source", "Company Name",
    "Current Stage", "Calling Date", "Phone No.", "Location", "Item/Qty"
  ];
  if (isAdmin()) directEnquiryHeaders.push("Assigned To");
  ...
  const renderPendingRow = (tracker, index) => (
    <tr key={tracker.id || index} className="hover:bg-slate-50">
      ...
      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{tracker.receiverName}</td>
      ...
      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{tracker.salespersonName}</td>
      ...
      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{tracker.callingDate}</td>
      {isAdmin() && <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{tracker.assignedTo}</td>}
      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
  ...
  const getHeaders = () => {
    if (activeTab === "pending") return pendingHeaders;
  ...
  const getRenderRow = () => {
    ...
    if (activeTab === "directEnquiry") return renderDirectEnquiryRow;
    return renderHistoryRow;
  };

  const getRenderCard = () => {
    if (activeTab === "pending") return renderPendingCard;
    if (activeTab === "directEnquiry") return renderDirectEnquiryCard;
