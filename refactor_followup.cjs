...
if (!content.includes('import DataTable from')) {
    content = content.replace(/(import \{ AuthContext \} from "\.\.\/\.\.\/App")/, '$1\nimport DataTable from "../../components/DataTable"\nimport FollowUpFilter from "../../components/follow-up/FollowUpFilter"');
}

const returnRegex = /  return \([\r\n]+    <div className="min-h-screen bg-gray-50">/;
const match = content.match(returnRegex);
const returnIndex = match ? match.index : -1;
...
const newContent = beforeReturn + newLogic;
fs.writeFileSync(filepath, newContent, 'utf8');
console.log("Successfully refactored FollowUp.jsx with Node!");
