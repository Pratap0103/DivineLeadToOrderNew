...
function FollowUp() {
  ...
  const filteredPendingFollowUps = pendingFollowUps.filter((followUp) => {
    ...
    const matchesFilterType = filterType.length === 0 || filterType.some((ft) => {
      if (ft === "first") {
    ...
    const matchesCompanyFilter = companyFilter.length === 0 || companyFilter.includes(followUp.companyName)

    // Apply person filter
    const matchesPersonFilter = personFilter.length === 0 || personFilter.includes(followUp.personName)
